...
inline fun <reified AutoRegistrableMarker : Registrable> autoRegister(registry: Registry<*>? = null) {
    ...
    AutoRegistrableMarker::class
        ...
        // Filter by annotation BEFORE objectInstance — otherwise client-only objects
        // (PartialModel, Screen, GuiGraphics, …) initialize on the dedicated server.
        .filter { kClass ->
            val ann = kClass.annotations.filterIsInstance<RegistrableEnv>().firstOrNull()
            ann == null || ann.value == platform.environment
        }.mapNotNull { it.objectInstance as? Registrable }
        .filter { registrable ->
