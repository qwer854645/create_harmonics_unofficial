...
abstract class PlatformEventBridge<PE : Any> {
    companion object {
        ...
        // Lazily built — walking ClientProxyEvent must not run during dedicated-server class init
        // of this companion (and validateAll(false) never needs it).
        private val clientTrackingLazy: MutableMap<KClass<out ClientProxyEvent>, Boolean> by lazy {
            ...
            if (missing.isNotEmpty()) {
                error(
                    "Unregistered proxy events!\n" +
                        ...
                        "\nhttps://github.com/qwer854645/create_resonance/issues",
                )
            }
        }
    }
